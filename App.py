...
from LaneDetection import detect_shapes  # Import frame processing from LineProcessing.py
import cv2
import time
...
def raw_video_feed():
    def generate_frames():
        capture = cv2.VideoCapture("AldreiC - RoadVideo.MOV")
        while capture:
...
def overlay_video_feed():
    def generate_frames():
        ...
        while capture:
            ...
            processed_frame = detect_shapes(image, arrow, height, width)

            if processed_frame is None:
                print("Error: Processed frame is None, using original frame")
                processed_frame = success  # Use the original frame if processing fails
